...
public class UserController {
    ...
    public String home(ModelMap model) {
        ...
        for (Product p : dsProduct) {
            Double minPrice = dsDetail.stream()
                                      .filter(detail -> detail.getProduct().getId() == p.getId()) 
                                      .mapToDouble(ProductDetail::getPrice)
                                      .min() // Lấy giá nhỏ nhất
	...
	public String search(@RequestParam(value = "search", required = false) String search, ModelMap model) {
	    ...
	    if (search != null && !search.isEmpty()) {
	        dsProduct = dsProduct.stream()
	                             .filter(p -> p.getName().toLowerCase().contains(search.toLowerCase())) // Lọc sản phẩm có tên chứa chuỗi search
	    ...
	    for (Product p : dsProduct) {
	        ...
	        Double minPrice = dsDetail.stream()
	                                  ...
	                                  .mapToDouble(ProductDetail::getPrice)
	                                  .min()
	                                  .orElse(0.0); // Giá mặc định nếu không tìm thấy
		    ...
		    if (cart.getCustomer().getId() == givenCustomerId) {
		        ...
		        if (productDetail != null) {
		            ...
		            Product product = dsProduct.stream()
		                .filter(p -> p.getId() == productDetail.getProduct().getId()) // so sánh ID
		                .findFirst()
		                .orElse(null);
    ...
    public String order(HttpSession ses, ModelMap model) {
		...
		for (OrderDetail orderDetail : dsOrdDetail) {
		    ...
		    Order order = dsOrder.stream()
		        .filter(o -> o.getId() == orderDetail.getOrder().getId()) // So sánh ID
		        .findFirst()
		        .orElse(null);
    ...
    public String productinfo(HttpServletRequest request) {
		...
		request.setAttribute("size",toRemove);
